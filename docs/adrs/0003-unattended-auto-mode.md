...
The hook URL contract pinned in Decision §1 (`http://localhost:<port>/hooks` with `<port>` taken from `AgentRegistry.port`) assumes the spawned CLI driver can reach the dashboard's HTTP listener directly on `localhost`. This holds for the standalone deployment where the dashboard, the session daemon, and the agent PTY all live on the same host — exactly the same assumption the existing MCP injection at `http://127.0.0.1:<port>/mcp` already relies on (`server-rs/src/agents/driver.rs:280`).
