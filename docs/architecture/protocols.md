...
| `AgentStopped { agent_id, status, cost_usd?, stop_reason? }` | yes | Agent process exited or was killed. Server updates the `agents` row, runs org-budget enforcement if `cost_usd` is set, and broadcasts `agent_stopped`. All three sender paths in `branchwork_runner.rs` (normal exit, spawn failure, explicit `KillAgent`) use `send_reliable`, so the event survives a transient WS drop between agent exit and ACK via outbox replay. |
