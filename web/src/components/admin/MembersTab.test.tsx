...
beforeEach(() => {
  ...
  vi.stubGlobal("fetch", async (input: RequestInfo, init?: RequestInit) => {
    ...
    if (url.startsWith("/api/orgs/acme/members/") && url.endsWith("/role") && method === "PUT") {
      return new Response(JSON.stringify({ ok: true }), { status: 200 });
    }
...
describe("MembersTab", () => {
  it("loads and renders members from /api/orgs/{slug}", async () => {
    render(<MembersTab orgSlug="acme" callerRole="owner" callerUserId="user-1" />);
  ...
  it("invite POSTs trimmed lowercase email + selected role", async () => {
    ...
    await waitFor(() => {
      const post = calls.find((c) => c.url === "/api/orgs/acme/members" && c.method === "POST");
  ...
  it("owner gets a role picker; non-owner gets a static role label", async () => {
    ...
    rerender(<MembersTab orgSlug="acme" callerRole="admin" callerUserId="user-1" />);
    await waitFor(() => expect(screen.getByTestId("members-list")).toBeTruthy());
  ...
  it("changing role PUTs to /role endpoint with new role", async () => {
    ...
    await waitFor(() => {
      const put = calls.find(
        (c) => c.url === "/api/orgs/acme/members/user-2/role" && c.method === "PUT",
      );
  ...
  it("Remove button DELETEs the member endpoint", async () => {
    ...
    await waitFor(() => {
      const del = calls.find(
        (c) => c.url === "/api/orgs/acme/members/user-2" && c.method === "DELETE",
      );
  ...
  it("hides invite form for non-admin callers (read-only)", async () => {
    render(<MembersTab orgSlug="acme" callerRole="member" callerUserId="user-2" />);
    await waitFor(() => expect(screen.getByTestId("members-list")).toBeTruthy());
