import { useEffect, useRef, useMemo, useState, useCallback, Suspense, lazy } from "react";
...
// xterm + addons add ~300 KB raw / ~70 KB gzipped to the bundle (audit §20).
// Only one agent is in PTY mode at a time, so the terminal is split out
// into its own chunk and pulled in on demand the first time the user
// opens a PTY agent. Stream-json check agents never trigger the load.
const PtyTerminal = lazy(() => import("./PtyTerminal.js"));
        ...
        isPty ? (
          <Suspense
            fallback={
              ...
              >
                <p className="text-xs text-gray-600">Loading terminal...</p>
              </div>
            }
          >
